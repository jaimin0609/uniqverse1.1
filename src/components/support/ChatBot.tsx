...
export default function ChatBot() {
        ...
        const loadConfig = async () => {
            try {
                ...
                if (data.success) {
                    ...
                    let welcomeMessage = data.config.welcome_message || 'Hello! 👋 Welcome to UniQVerse support! I\'m your AI assistant here to help you with any questions about our products, orders, shipping, returns, and more. How can I assist you today?';

                    if (session?.user?.name) {
                        welcomeMessage = `Hello ${session.user.name}! 👋 Welcome back to UniQVerse support! I'm your AI assistant and I can help you with your orders, products, shipping, returns, and more. How can I assist you today?`;
                    } else if (session?.user) {
                        welcomeMessage = `Hello! 👋 Welcome back to UniQVerse support! I'm your AI assistant and I can help you with your orders, products, shipping, returns, and more. How can I assist you today?`;
                    }

                    // Initialize messages with the personalized welcome message
                    setMessages([{
                        role: "assistant",
                ...
                // Use personalized fallback message
                let fallbackMessage = 'Hello! 👋 Welcome to UniQVerse support! I\'m your AI assistant here to help you with any questions about our products, orders, shipping, returns, and more. How can I assist you today?';

                if (session?.user?.name) {
                    fallbackMessage = `Hello ${session.user.name}! 👋 Welcome back to UniQVerse support! I'm your AI assistant here to help you with any questions. How can I assist you today?`;
                }
    ...
    async function onSubmit(values: z.infer<typeof messageSchema>) {
        ...
        try {
            ...
            setShowQuickActions(false);            // Choose API endpoint based on user authentication
            const apiEndpoint = session?.user ? "/api/ai-chat-personalized" : "/api/ai-chat";

            // Send message to appropriate AI API
            ...
            let assistantMessage: Message; if (session?.user && data.userAuthenticated) {
                ...
                if (data.userName && messages.length === 1) {
                    // First interaction - show personalized greeting
                    assistantMessage.content = data.message;
                }
            } else {
                // Standard API response
                assistantMessage = {
                    id: data.messageId || `standard_${Date.now()}`,
                    role: "assistant",
    ...
    async function provideFeedback(messageId: string | undefined, isHelpful: boolean) {
        ...
        // Handle special quick actions for guest users
        if (!session?.user && suggestion === "Login for order help") {
        ...
        // Create personalized welcome message
        let welcomeMessage = "Hello! 👋 Welcome to UniQVerse support! I'm your AI assistant here to help you with any questions about our products, orders, shipping, returns, and more. How can I assist you today?";
    ...
    const handleChatClose = useCallback(async () => {
        ...
        setIsMinimized(false);
        setUnreadCount(0);
    }, [userInteractionData.messageId, submitSmartRating]);    // Quick action buttons data - different for authenticated vs guest users
    const quickActions = session?.user ? [
        { label: "Track my order", icon: "📦" },
        { label: "My recent orders", icon: "📋" },
        ...
        { label: "Login for order help", icon: "�" }
    ]; return (
                ...
                >
                    ...
                    <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-4 rounded-t-2xl flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h3 className="font-semibold text-lg">{config.chatbot_name}</h3>                                    <div className="flex items-center gap-2 text-sm opacity-90">
                                    <Sparkles className="h-3 w-3" />
                                    <span>
                                        {session?.user
                                            ? `Personalized • ${session.user.name ? `Hello ${session.user.name}` : 'Logged In'}`
                                            : 'AI-Powered • Online'
                                        }
                                    </span>
                                </div>
                        ...
                        <>
                            ...
                            <div className="border-t border-gray-200 p-4 bg-white rounded-b-2xl">
                                ...
                                <div className="mt-3 flex items-center justify-between text-xs text-gray-500">
                                    ...
                                    <div className={`flex items-center gap-1 text-gray-400 ${styles.gradientText}`}>                                            <Sparkles className="h-3 w-3" />
