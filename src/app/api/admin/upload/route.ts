...
export async function POST(request: NextRequest) {
    try {
        ...
        // Define the folder structure in iDrive e2
        const folderPath = `products/${new Date().getFullYear()}/${(new Date().getMonth() + 1).toString().padStart(2, '0')}`;
        ...
        await logAdminAction(
            "media_upload",
            `Admin uploaded media file "${file.name}" (ID: ${media.id})`,
            session.user.id
        );
